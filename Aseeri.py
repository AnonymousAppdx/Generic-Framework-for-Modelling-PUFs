...
## k=5 seed=52 N=350k fail

PUF_list.add_XOR_PUF(k=k,num=1)
...
x = tf.keras.layers.Dense(2**k, activation=activation,kernel_initializer=kernel_init)(input_layer)
x = tf.keras.layers.Dense(2**(k), activation=activation,kernel_initializer=kernel_init)(x)
x = tf.keras.layers.Dense(2**k, activation=activation,kernel_initializer=kernel_init)(x)
...
model.summary()
# Train
batch_size = min(N_crp[k-2]//40,40000)
...
# 打印内存使用情况
print(f"Memory cost: {(memory_info.rss-memory_info_start.rss) / (1024 ** 3):.2f} GiB")